...
struct sceneObj
{
	...
	jhl_rgb		color;
	bool		visible = true;		// 表示フラグ（キー1,2,3で切り替え）
};
...
static void obj_attribs_init()
{
	...
	p_objTgt->trans = 1;
	p_objTgt->acc = 1;
	p_objTgt->acc.rot_axis_x(0.02f);  // X軸回転を追加して表面が見えるように
...
int proc_key(char key)
{
	...
	if (key == last_toggle_key) {
		return 0;  // 同じキーが押され続けている間は無視
	}
	// トグル系キー ('1', '2', '3') の場合のみ記録
	if (key == '1' || key == '2' || key == '3') {
		last_toggle_key = key;
	} else {
		last_toggle_key = 0;  // 他のキーでリセット
	}
